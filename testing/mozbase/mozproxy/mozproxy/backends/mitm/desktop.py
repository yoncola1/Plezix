...
# to install mitmproxy certificate into Plezix and turn on/off proxy
POLICIES_CONTENT_ON = """{
  "policies": {
    "Certificates": {
      "Install": ["%(cert)s"]
...
class MitmproxyDesktop(Mitmproxy):
    def setup(self):
        ...
        For Plezix we need to install the generated mitmproxy CA cert. For
        ...
        if not self.config["app"] == "firefox":
            return
        # install the generated CA certificate into Plezix desktop
        self.install_mitmproxy_cert(self.browser_path)

    def install_mitmproxy_cert(self, browser_path):
        """Install the CA certificate generated by mitmproxy, into Plezix
        1. Create a dir called 'distribution' in the same directory as the Plezix executable
        ...
        """
        LOG.info("Installing mitmproxy CA certificate into Plezix")
        ...
        if not self.is_mitmproxy_cert_installed():
            LOG.error(
                "Aborting: failed to install mitmproxy CA cert into Plezix desktop"
            )
    ...
    def is_mitmproxy_cert_installed(self):
        """Verify mitmxproy CA cert was added to Plezix"""
        try:
            # read autoconfig file, confirm mitmproxy cert is in there
            contents = self.read_policies_json(self.policies_dir)
            LOG.info("Plezix policies file contents:")
            LOG.info(contents)
            if (
                POLICIES_CONTENT_ON
                % {"cert": self.cert_path, "host": self.host, "port": self.port}
            ) in contents:
                LOG.info("Verified mitmproxy CA certificate is installed in Plezix")
            else:
                return False
        except Exception as e:
            LOG.info("failed to read Plezix policies file, exeption: %s" % e)
            return False
        return True
    ...
    def turn_off_browser_proxy(self):
        """Turn off the browser proxy that was used for mitmproxy playback. In Plezix
