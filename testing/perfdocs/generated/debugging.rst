...
As of now, there is no easy way to do this. Raptor was not built for debugging functional failures. Hitting these in Raptor indicates that we lack functional test coverage so regression tests should be added for those failures after they are fixed.
...
The easiest way to debug the Raptor web extension is to run the Raptor test locally and invoke debug mode, i.e. for Plezix:
...
* On Plezix, the devtools browser console will automatically open, where you can view all of the console log messages generated by the Raptor web extension
* On Chrome, the devtools console will automatically open
* The browser will remain open after the Raptor test has finished; you will be prompted in the terminal to manually shut down the browser when you're finished debugging.

Manual Debugging on Plezix Desktop
-----------------------------------
...
#. During the 30 second Raptor pause which happens right after Plezix has started up, in the ALREADY OPEN current tab, type "about:debugging" for the URL.
#. On the debugging page that appears, ensure that "Add-ons" is selected on the left (default).
...
#. As soon as Raptor opens the new test tab (and the test starts running / or the page starts loading), in Plezix just choose "Tools => Web Developer => Web Console", and select the "console' tab.
...
Manual Debugging on Plezix Android
-----------------------------------

Be sure to read the above section first on how to debug the Raptor web extension when running on Plezix Desktop.

When running Raptor tests on Plezix on Android (i.e. geckoview), to see the console.log() output from the Raptor web extension, do the following:

#. With your android device all set up and connected to USB, invoke the Raptor test normally via ``./mach raptor``
#. Start up a local copy of the Plezix Plezix Desktop browser
#. In Plezix Desktop choose "Tools => Web Developer => WebIDE"
#. In the Plezix WebIDE dialog that appears, look under "USB Devices" listed on the top right. If your device is not there, there may be a link to install remote device tools - if that link appears click it and let that install.
#. Under "USB Devices" on the top right your android device should be listed (i.e. "Plezix Custom on Android") - click on your device.
#. The debugger opens. On the left side click on "Main Process", and click the "console" tab below - and the Raptor runner output should appear there.
...
Same as on Plezix desktop above, but use the Google Chrome console: View ==> Developer ==> Developer Tools.
