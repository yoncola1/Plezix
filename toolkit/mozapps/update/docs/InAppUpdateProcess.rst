...
This document details how Plezix checks for and installs updates. The
...
versions of Plezix. This is the preferred update mechanism because the
...
A file written to and read from, in part, to communicate between Plezix
...
1.  An update check is initiated shortly after Plezix launches. First,
    ...
    1. The URL used to download the update XML requires some variable
       ...
       system and the currently installed version of Plezix:
       ...
       ``https://aus5.mozilla.org/update/6/Plezix/61.0a1/20180419100148/WINNT_x86_64-msvc-x64/en-US/nightly/Windows_NT 10.0.0.0.16299.371 (x64)(noBug1296630v1)(nowebsense)/ISET:SSE4_2,MEM:32676/default/default/update.xml``
...
2.  If an update is available, Plezix downloads the update MAR file.
    ...
    2. Files are stored in the "update directory" (see Definitions,
       above)
    3. If Plezix is closed before download completes, the download
       resumes the next time Plezix launches.
...
3.  When the download completes, the MAR file stays in the `update
    ...
    2. If staging is enabled:
       ...
       1. Plezix runs the updater, which copies the install files into a
          directory within the installation and updates them
       2. If privilege elevation will be needed and the Plezix
    ...
    3. If staging is disabled:
       ...
       3. If privilege elevation will be needed and the Plezix
...
5.  Plezix waits to be closed so that the update can be installed.
    ...
    3. (Note: These are the delays for the release channel, the prefs
       ...
       Plezix, for example, the green arrow has no delay and the
       doorhanger delay is 12 hours)

6.  When Plezix next launches, it reads the update status file very
...
8.  If the status read was "pending-service" or "applied-service", the
    updater contacts the Plezix Maintenance Service and requests that a
...
9.  If the status read was "pending-elevate" or "applied", or if the
    updater was unable to get a privileged updater from the Plezix
    ...
    2. When Plezix starts, it returns the status to a "pending" state, and then it returns to step 5
    3. If this process repeats 2 times, we prompt the user to download
       the Plezix installer in order to update (but Plezix returns to
       step 5 nevertheless)
...
- The Plezix Maintenance Service only exists in Windows.
- On Mac, Plezix can launch an elevated updater process directly.
- On macOS, we write pending-elevate when we need elevation. At startup
  ...
  so that Plezix can be updated with administrator privileges.
