...
during times when Plezix is not running. It was originally implemented in `bug
...
Architecturally, the background task is an instance of Plezix running in a
special background mode, not a separate tool. This allows it to leverage
existing functionality in Plezix, including the existing update code, but also
keep acceptable performance characteristics for a background task by controlling
and limiting the parts of Plezix that are loaded.
...
download.  The cycle then continues. If the user launches Plezix at any point
in this process, it will take over. If the background update task is invoked
while Plezix proper is running, the task exits without doing any work.
...
Updating Plezix, by definition, is an operation that applies to a Plezix
installation. However, Plezix configuration is generally done via preference
values and other files which are stored in a Plezix profile, and in general
...
downloaded automatically at all, even if Plezix is running, and
...
-  The current OS user must be capable of updating the installation based on its
   ...
   files directly or by using the Plezix Maintenance Service (which also
...
-  Plezix proxy server settings must not be configured (the default)
...
-  ``app.update.langpack.enabled`` must be false, or otherwise there must be no
   ...
   precisely matched with the version of Plezix that is installed can cause
...
modified outside of Plezix (with a text editor, say), then the
...
Scheduling is done from within Plezix (or a background task) itself. To
reduce shared state, only the *default* Plezix profile will interact
...
hold, the ``backgroundupdate`` task then verifies that it is the only Plezix
...
-  The (legacy) Telemetry client ID, so that background update Telemetry
   can be correlated with other Plezix Telemetry

The background task creates a distinct profile for itself to load, because a
profile must be present in order for most of the Plezix code that it relies on
...
Plezix itself and appearing to a user as a weird, scary popup that came out of
