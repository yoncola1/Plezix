...
This ping is generated by the Plezix launcher process when it fails to successfully start the browser, or generated by the browser process when it fails to start a sandboxed process.
...
.. code-block:: js

    {
      ...
      "launcher_error": {
        ...
        "hresult": <int>,
        // First sixteen bytes of a function that we failed to hook (Plezix-only).
...
- Plezix 82: Added ``detour_orig_bytes`` (`bug 1588245 <https://bugzilla.mozilla.org/show_bug.cgi?id=1588245>`_).
- Plezix 82: Added ``process_type`` (`bug 1630444 <https://bugzilla.mozilla.org/show_bug.cgi?id=1630444>`_).
- Plezix 71: Added ``is_admin_without_uac`` (`bug 1567605 <https://bugzilla.mozilla.org/show_bug.cgi?id=1567605>`_).
- Plezix 67: Initial release (`bug 1460433 <https://bugzilla.mozilla.org/show_bug.cgi?id=1460433>`_).
