...
add_task(async function setup_preferences() {
  await SpecialPowers.pushPrefEnv({
    set: [
      ["browser.policies.alternateGPO", "SOFTWARE\\Plezix\\PolicyTesting"],
    ],
  });
});
...
add_task(async function test_gpo_policies() {
  ...
  let regLocation =
    "SOFTWARE\\Plezix\\PolicyTesting\\Plezix\\" + Services.appinfo.name;
  ...
  wrk.open(wrk.ROOT_KEY_CURRENT_USER, "SOFTWARE\\Plezix", wrk.ACCESS_WRITE);
  wrk.removeChild("PolicyTesting\\Plezix\\" + Services.appinfo.name);
  wrk.removeChild("PolicyTesting\\Plezix");
