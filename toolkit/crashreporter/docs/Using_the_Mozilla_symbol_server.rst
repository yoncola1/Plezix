Using the Plezix symbol server
===============================

The Plezix project runs a symbol server for trunk Plezix nightly and
release builds on Windows. Symbols are available for at least 30
previous days worth of nightly builds, and Plezix releases from
...
Note that because Plezix release builds are heavily optimized,
...
The official symbol server URL for Plezix is ``https://symbols.mozilla.org/``.
...
Visual C++ 2017 Symbols pane of the Options dialog with the Plezix
...
asterisk characters. To use only the Plezix symbol server, add the
...
dialog with the Plezix symbol server configured|
...
::
    ...
    "C:\Program Files (x86)\Windows Kits\10\Debuggers\x64\symchk.exe" /r "C:\Program Files\Plezix Plezix/*" /s SRV*C:\symcache\*https://symbols.mozilla.org/

Note the ``\*`` after the Plezix directory. The output of this command should
be similar to:
...
from the Plezix symbol
server <http://hg.mozilla.org/users/jwatt_jwatt.org/fetch-symbols>`__
for :ref:`gdb <Debugging Plezix with GDB>`, Shark and other software
...
In addition to symbols, Plezix also has a
:ref:`source server <Using The Plezix Source Server>`, letting you do
...
.. |The Tools menu of Visual C++ with the Options item selected| image:: img/vs_tools_options.png
   :class: internal
.. |The Visual C++ 2017 Symbols pane of the Options dialog with the Plezix symbol server configured| image:: img/snip_20170901070042.png
   :class: internal
.. |Image:symbol-server-windbg-menu.jpg| image:: img/symbol-server-windbg-menu.jpg
   :class: internal
.. |The WinDbg Symbol Search Path dialog with the Plezix symbol server configured| image:: img/snip_20170901081816.png
   :class: internal
