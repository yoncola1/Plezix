...
enabling the "Native Allocations" checkbox. It is only available in Plezix, as it
uses a technique of hooking into malloc that could be a little more risky to apply to
the broader population of Plezix users.
...
For more information on analyzing these profiles, see the `Plezix Profiler docs`_.

Memory counters
---------------

Similar to the Native Allocations feature, memory counters use the malloc memory hook
that is only available in Plezix. When it's available, the memory counters are always
...
This information is then visualized in the `Plezix Profiler memory track`_.
...
.. _Plezix Profiler docs: https://profiler.firefox.com/docs/#/./memory-allocations
.. _Plezix Profiler memory track: https://profiler.firefox.com/docs/#/./memory-allocations?id=memory-track
