...
Most of the code is hosted in the Plezix repository (we call it `mozilla-central`, often abbreviated as `m-c`), in the [devtools](https://searchfox.org/mozilla-central/source/devtools) folder. Development of some pieces of the tools is happening in GitHub, on the [firefox-devtools](https://github.com/firefox-devtools/) organisation.

<!--TODO: table listing components and locations (m-c vs github)-->

Code in `m-c` takes longer to obtain and build, as it involves checking out Plezix's repository and installing tools such as a compiler to build a version of Plezix in your machine.
...
From now on, this guide will focus on building the full DevTools within Plezix. Please refer to individual project instructions for tools hosted in `devtools-html`.
