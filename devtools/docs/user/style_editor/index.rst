...
From Plezix 40 onwards, the style sheet pane also includes a context menu that lets you open the selected style sheet in a new tab.
...
From Plezix 46 onwards, if an ``@media`` rule contains a screen size in a condition, then it is made clickable: clicking it then resizes the screen to that size using the Responsive Design View.
...
Source maps enable the tools to map back from the generated CSS to the original syntax, so they can display, and allow you to edit, files in the original syntax. From Plezix 29 onwards, the Style Editor can understand CSS source maps.
...
From Plezix 35 onwards original sources are displayed by default.
