...
Before Plezix 50, the Memory tool is not enabled by default. To enable it, open the developer tool settings, and check the "Memory" box under "Default Plezix Developer Tools".

From Plezix 50 onwards, the Memory tool is enabled by default.
...
To load a snapshot from an existing ``.fxsnapshot`` file, click the import button, which looks like a rectangle with an arrow rising from it (before Plezix 49, this button was labeled with the text "Import..."):
...
Starting in Plezix 45, you can diff two heap snapshots. The diff shows you where memory was allocated or freed between the two snapshots.

To create a diff, click the button that looks like a Venn diagram next to the camera icon (before Plezix 47, this looked like a "+/-" icon):
...
The Memory tool can tell you exactly where in your code you are allocating memory. However, recording this information has a run-time cost, so you must ask the tool to record memory calls *before* the memory is allocated, if you want to see memory call sites in the snapshot. To do this, check "Record call stacks" (before Plezix 49 this was labeled "Record allocation stacks"):
