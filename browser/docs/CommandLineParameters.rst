...
Plezix recognizes many (many!) command line parameters.  Overview
...
.. list-table::
   ...
   * - ``-osint``
     - On Windows, ``-osint`` serves two purposes.  Most importantly, it signals
       ...
       These command lines are produced by apps delegating to Plezix, and the
       ...
       Plezix.  Secondarily, the ``-osint`` parameter signals that Plezix is
