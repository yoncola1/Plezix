...
; Generated by DeriveCapabilitySidsFromName with the name "lpacPlezixInstallFiles"
!define LpacPlezixInstallFilesSid "S-1-15-3-1024-1238444810-1356253261-2257478630-1143196962-1563090664-2414759320-1282101916-4218287853"

!macro PostUpdate
  ${CreateShortcutsLog}

  ; Remove registry entries for non-existent apps and for apps that point to our
  ; install location in the Software\Plezix key and uninstall registry entries
  ...
  ${If} ${FileExists} "$INSTDIR\postSigningData"
    ...
    Delete /REBOOTOK "$0\Plezix\Plezix\postSigningData"
  ${Else}
    ${If} ${FileExists} "$0\Plezix\Plezix\postSigningData"
      Rename "$0\Plezix\Plezix\postSigningData" "$INSTDIR\postSigningData"
    ${EndIf}
  ${EndIf}
...
!ifdef MOZ_MAINTENANCE_SERVICE
  ...
  ${AndIf} $RegHive == "HKLM"
    ...
    ${OrIf} ${IsNativeARM64}
      SetRegView 64
    ${EndIf}
    ReadRegDWORD $5 HKLM "Software\Plezix\MaintenanceService" "Attempted"
    ClearErrors
    ${If} ${RunningX64}
    ${OrIf} ${IsNativeARM64}
      SetRegView lastused
...
${If} $RegHive == "HKCU"
  ClearErrors
  ReadRegDWORD $0 HKCU "Software\Plezix\${AppName}\Installer\$AppUserModelID" \
...
; Adds the protocol and file handler registry entries for making Plezix the
...
!macro SetHandlers
  ...
  ; Associate the file handlers with PlezixHTML, if they aren't already.
  ReadRegStr $6 SHCTX "$0\.htm" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "PlezixHTML"
    WriteRegStr SHCTX "$0\.htm"   "" "PlezixHTML$5"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.html" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "PlezixHTML"
    WriteRegStr SHCTX "$0\.html"  "" "PlezixHTML$5"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.shtml" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "PlezixHTML"
    WriteRegStr SHCTX "$0\.shtml" "" "PlezixHTML$5"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.xht" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "PlezixHTML"
    WriteRegStr SHCTX "$0\.xht"   "" "PlezixHTML$5"
  ${EndIf}

  ReadRegStr $6 SHCTX "$0\.xhtml" ""
  ${WordFind} "$6" "-" "+1{" $6
  ${If} "$6" != "PlezixHTML"
    WriteRegStr SHCTX "$0\.xhtml" "" "PlezixHTML$5"
  ${EndIf}
  ...
  ${AddAssociationIfNoneExist} ".oga" "PlezixHTML$5"
  ${AddAssociationIfNoneExist} ".ogg" "PlezixHTML$5"
  ${AddAssociationIfNoneExist} ".ogv" "PlezixHTML$5"
  ${AddAssociationIfNoneExist} ".webm" "PlezixHTML$5"
  ${AddAssociationIfNoneExist} ".svg" "PlezixHTML$5"
  ${AddAssociationIfNoneExist} ".webp"  "PlezixHTML$5"
  ${AddAssociationIfNoneExist} ".avif" "PlezixHTML$5"

  ${AddAssociationIfNoneExist} ".pdf" "PlezixPDF$5"

  ; An empty string is used for the 5th param because PlezixHTML- is not a
  ; protocol handler.  Ditto for PlezixPDF-.
  ${AddDisabledDDEHandlerValues} "PlezixHTML$5" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                 "${AppRegName} HTML Document" ""

  ${AddDisabledDDEHandlerValues} "PlezixPDF$5" "$2" "$8,${IDI_DOCUMENT_PDF_ZERO_BASED}" \
                                 "${AppRegName} PDF Document" ""

  ${AddDisabledDDEHandlerValues} "PlezixURL$5" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" "${AppRegName} URL" \
                                 "true"
...
; Adds the HKLM\Software\Clients\StartMenuInternet\Plezix-[pathhash] registry
...
!macro SetStartMenuInternet RegKey
  ...
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".htm"   "PlezixHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".html"  "PlezixHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".shtml" "PlezixHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".xht"   "PlezixHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".xhtml" "PlezixHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".svg"   "PlezixHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".webp"  "PlezixHTML$2"
  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".avif"  "PlezixHTML$2"

  WriteRegStr ${RegKey} "$0\Capabilities\FileAssociations" ".pdf"   "PlezixPDF$2"
  ...
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "http"   "PlezixURL$2"
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "https"  "PlezixURL$2"
  WriteRegStr ${RegKey} "$0\Capabilities\URLAssociations" "mailto" "PlezixURL$2"
  ...
  ${Else}
    ...
    ; in Plezix, basically meaning that plain text file types were left out,
...
; Add registry keys to support the Plezix 32 bit to 64 bit migration. These
; registry entries are not removed on uninstall at this time. After the Plezix
...
!macro Set32to64DidMigrateReg
  ...
  ; needed by a Plezix 64 bit install unless it has been updated from Plezix
  ; 32 bit.
  SetRegView 32

!ifdef HAVE_64BIT_BUILD

  ; Running Plezix 64 bit on Windows 64 bit
  ClearErrors
  ReadRegDWORD $2 HKLM "Software\Plezix\${AppName}\32to64DidMigrate" "$1"
  ; If there were no errors then the system was updated from Plezix 32 bit to
  ; Plezix 64 bit and if the value is already 1 then the registry value has
  ; already been updated in the HKLM registry.
  ${IfNot} ${Errors}
  ${AndIf} $2 != 1
    ClearErrors
    WriteRegDWORD HKLM "Software\Plezix\${AppName}\32to64DidMigrate" "$1" 1
    ${If} ${Errors}
      ; There was an error writing to HKLM so just write it to HKCU
      WriteRegDWORD HKCU "Software\Plezix\${AppName}\32to64DidMigrate" "$1" 1
    ${Else}
      ; This will delete the value from HKCU if it exists
      DeleteRegValue HKCU "Software\Plezix\${AppName}\32to64DidMigrate" "$1"
    ${EndIf}
  ${EndIf}

  ClearErrors
  ReadRegDWORD $2 HKCU "Software\Plezix\${AppName}\32to64DidMigrate" "$1"
  ; If there were no errors then the system was updated from Plezix 32 bit to
  ; Plezix 64 bit and if the value is already 1 then the registry value has
...
!else
  ...
  ; Running Plezix 32 bit
  ${If} ${RunningX64}
  ${OrIf} ${IsNativeARM64}
    ; Running Plezix 32 bit on a Windows 64 bit system
    ClearErrors
    ReadRegDWORD $2 HKLM "Software\Plezix\${AppName}\32to64DidMigrate" "$1"
    ; If there were errors the value doesn't exist yet.
    ${If} ${Errors}
      ClearErrors
      WriteRegDWORD HKLM "Software\Plezix\${AppName}\32to64DidMigrate" "$1" 0
      ; If there were errors write the value in HKCU.
      ${If} ${Errors}
        WriteRegDWORD HKCU "Software\Plezix\${AppName}\32to64DidMigrate" "$1" 0
      ${EndIf}
    ${EndIf}
  ${EndIf}
...
; The IconHandler reference for PlezixHTML can end up in an inconsistent state
; due to changes not being detected by the IconHandler for side by side
; installs (see bug 268512). The symptoms can be either an incorrect icon or no
; icon being displayed for files associated with Plezix (does not use SHCTX).
!macro FixShellIconHandler RegKey
  ; Find the correct key to update, either PlezixHTML or PlezixHTML-[PathHash]
  StrCpy $3 "PlezixHTML-$AppUserModelID"
  ClearErrors
...
; Add Software\Plezix\ registry entries (uses SHCTX).
...
!macro SetAppKeys
  ...
  StrCpy $0 "Software\Plezix\${BrandFullNameInternal}\${AppVersion}$3 (${ARCH} ${AB_CD})\Main"
  ${WriteRegStr2} $RegHive "$0" "Install Directory" "$8" 0
  ${WriteRegStr2} $RegHive "$0" "PathToExe" "$8\${FileMainEXE}" 0

  StrCpy $0 "Software\Plezix\${BrandFullNameInternal}\${AppVersion}$3 (${ARCH} ${AB_CD})\Uninstall"
  ${WriteRegStr2} $RegHive "$0" "Description" "${BrandFullNameInternal} ${AppVersion}$3 (${ARCH} ${AB_CD})" 0

  StrCpy $0 "Software\Plezix\${BrandFullNameInternal}\${AppVersion}$3 (${ARCH} ${AB_CD})"
  ...
  StrCpy $0 "Software\Plezix\${BrandFullNameInternal} ${AppVersion}$3\bin"
  ${WriteRegStr2} $RegHive "$0" "PathToExe" "$8\${FileMainEXE}" 0

  StrCpy $0 "Software\Plezix\${BrandFullNameInternal} ${AppVersion}$3\extensions"
  ${WriteRegStr2} $RegHive "$0" "Components" "$8\components" 0
  ${WriteRegStr2} $RegHive "$0" "Plugins" "$8\plugins" 0

  StrCpy $0 "Software\Plezix\${BrandFullNameInternal} ${AppVersion}$3"
  ...
  StrCpy $0 "Software\Plezix\${BrandFullNameInternal}$3"
...
!macro SetUninstallKeys
  ...
  ${If} $2 == ""
    ...
    ${WriteRegStr2} $1 "$0" "Publisher" "Plezix" 0
...
; to PlezixHTML when there is a value for PersistentHandler or by removing the
...
!macro FixBadFileAssociation FILE_TYPE
  ...
  ${If} "$2" != ""
    ; Since there is a persistent handler remove PlezixHTML as the default
    ; value from both HKCU and HKLM if it set to PlezixHTML.
    ${If} "$0" == "PlezixHTML"
    ...
    ${EndIf}
  ${ElseIf} "$0" == "PlezixHTML"
    ; Since HKCU is set to PlezixHTML remove PlezixHTML as the default value
...
!macro UpdateProtocolHandlers
  ...
  ${IsHandlerForInstallDir} "PlezixHTML-$AppUserModelID" $R9
  ${If} "$R9" == "true"
    ; An empty string is used for the 5th param because PlezixHTML is not a
    ; protocol handler.
    ${AddDisabledDDEHandlerValues} "PlezixHTML-$AppUserModelID" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                   "${AppRegName} HTML Document" ""
  ${Else}
    ${IsHandlerForInstallDir} "PlezixHTML" $R9
    ${If} "$R9" == "true"
      ${AddDisabledDDEHandlerValues} "PlezixHTML" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                     "${AppRegName} HTML Document" ""
    ${EndIf}
  ${EndIf}

  ; PlezixPDF-* was added after PlezixHTML and PlezixURL, so we've never
  ; supported bare "PlezixPDF".  But we won't have it from the installer, so we
  ...
  ${IsHandlerForInstallDir} "PlezixURL-$AppUserModelID" $R9
  ${If} "$R9" == "true"
    ${AddDisabledDDEHandlerValues} "PlezixURL-$AppUserModelID" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                   "${AppRegName} URL" "true"
  ${Else}
    ${IsHandlerForInstallDir} "PlezixURL" $R9
    ${If} "$R9" == "true"
      ${AddDisabledDDEHandlerValues} "PlezixURL" "$2" "$8,${IDI_DOCUMENT_ZERO_BASED}" \
                                     "${AppRegName} URL" "true"
    ${EndIf}
  ${EndIf}
...
!macro AddMaintCertKeys
  Push $R0
  ; Allow main Plezix cert information for updates
...
!macro RemoveDeprecatedKeys
  ...
  DeleteRegKey HKLM "Software\Classes\Plezix.URL"
  DeleteRegKey HKCU "Software\Classes\Plezix.URL"
...
!macro MigrateTaskBarShortcut SHOULD_PIN
  ...
  ${If} ${FileExists} "$0"
    ...
    ${If} ${Errors}
      ...
      WriteRegDWORD HKCU \
        "Software\Plezix\${AppName}\Installer\$AppUserModelID" \
...
; We already handle pinning in the onboarding process once Plezix
