...
class SelectableProfileServiceClass extends EventEmitter {
  ...
  async #init(isInitial = false) {
    ...
    // This can happen if profiles.ini has been reset by a version of Plezix
  ...
  /*
   * Helper that executes a new Plezix process. Mostly useful for mocking in
   * unit testing.
   */
  execProcess(aArgs) {
    let executable =
  ...
  /**
   * Launch a new Plezix instance using the given selectable profile.
   *
   ...
   *   process, "remote" meaning the change was made by a different Plezix
  ...
  async maybeSetupDataStore() {
    ...
    if (!this.#currentProfile) {
      ...
      // launched second Plezix instance's dock animation to complete. Then
...
export class CommandLineHandler {
  ...
  handle(cmdLine) {
    ...
    // On macOS requests to open URLs from other applications in an already running Plezix are
