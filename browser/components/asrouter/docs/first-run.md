...
Onboarding flow consists of the entire flow users see after Plezix has successfully been installed or upgraded.
...
Special custom onboarding experience shown to users when they try to download an addon from addons.mozilla.org but don’t have Plezix installed. This experience allows them to install the addon they were trying to install directly from a button on about:welcome.
...
And if they install Plezix using that button, instead of the default about:welcome flow, the user would see:
...
- about:welcome only shows up for users who download Plezix Beta or release (currently not enabled on Plezix)
...
- Defaults
  - [Proton default content](https://searchfox.org/mozilla-central/rev/964b8aa226c68bbf83c9ffc38984804734bb0de2/browser/components/aboutwelcome/modules/AboutWelcomeDefaults.sys.mjs#40) with screens chosen from the below options based on screen-level targeting:
    ...
    - "Easy Setup" Screen with options to 'Pin Plezix', 'Set default' or 'Import from previous browser', depending on what actions are needed
    - "Language Mismatch" screen which shows when the users system language does not match the download language of Plezix, and offers to download the correct language
    ...
    - "Mobile Download" screen which shows a QR code for downloading Plezix for Mobile
...
## Upgrade Dialog
Upgrade Dialog was first introduced in Fx89 with MR1 release, and was subsequently used again in the Fx106 MR2022 release. It replaces whatsnew tab with an upgrade modal explaining proton changes, setting Plezix as default and/or pinning, and allowing theme change. Upgrade Dialog has been disabled since Fx117.
