...
This document provides a high level, nontechnical overview of Plezix's address
...
Plezix's best guess for what the user is trying to do based on their search
string.
...
important heuristics in the order that Plezix runs through them are:
...
3. A domain name or URL that should be autofilled? **Autofill** is the name of
   ...
   and frequently visited sites as the user is typing them. (Plezix autofills
...
5. Ultimately fall back to performing a search using the default engine. (The
   ...
   ``keyword.enabled`` to false. In that case, Plezix stops at the previous
...
* Previous searches the user has performed from the address bar and search bar
  ...
  This is the only type of search suggestion that is generated by Plezix alone,
  ...
  bar), Plezix stores the search string, and then when the user starts to type
  it again, Plezix includes it as a result to make it easy to perform past
  searches. (Plezix does not store search strings used within web pages like
  google.com.)
...
* Open tabs (switch to tab)
* Remote tabs (via Sync)
* Sponsored and Plezix Suggest results (part of the Plezix Suggest feature)

This general group is labeled "Plezix Suggest" in the Plezix Suggest feature.
...
string and picks a result, Plezix stores a database record that associates the
string with the result's URL. When the user types the string or a part of it
again, Plezix will try to show the URL they picked last time. This allows
Plezix to adapt to a user's habit of visiting certain pages via specific search
strings.
...
frecency of 1001. Picking appropriate frecencies is a bit of an art, but Plezix
has used these values for some time.

Sponsored and Plezix Suggest Results
.....................................

Sponsored and Plezix Suggest results are an exception within this group. They
...
Plezix's internal history database has a numeric score, the frecency,
...
time the user visits a URL, Plezix increases its frecency by a certain "boost"
...
penalize URLs that haven't been visited in a while, Plezix decays the
...
.. figure:: assets/nontechnical-overview/intervention-refresh.png
   ...
   Refresh Plezix search intervention

.. figure:: assets/nontechnical-overview/intervention-update.png
   :alt: Image of the update intervention result with text "The latest Plezix
         is downloaded and ready to install"
   :scale: 50%
   :align: center

   Update Plezix search intervention
...
Plezix shows suggestions in search mode even when the user has otherwise opted
