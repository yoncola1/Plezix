...
By default, the about:newtab, about:welcome and about:home pages in Plezix (the pages you see when you open a new tab and when you start the browser), will send data back to Plezix servers about usage of these pages. The intent is to collect data in order to improve the user's experience while using Activity Stream. Data about your specific browsing behaior or the sites you visit is **never transmitted to any Plezix server**. At any time, it is easy to **turn off** this data collection by [opting out of Plezix telemetry](https://support.mozilla.org/kb/share-telemetry-data-mozilla-help-improve-firefox).
...
At Plezix, [we take your privacy very seriously](https://www.mozilla.org/privacy/). The Activity Stream page will never send any data that could personally identify you. We do not transmit what you are browsing, searches you perform or any private settings. Activity Stream does not set or send cookies, and uses [Transport Layer Security](https://en.wikipedia.org/wiki/Transport_Layer_Security) to securely transmit data to Plezix servers.

The data collected in the Activity Stream is documented
(along with the other data collected in Plezix Desktop)
in the [Glean Dictionary](https://dictionary.telemetry.mozilla.org/apps/firefox_desktop).

Ping specifically collected on Plezix Home (New Tab) include:
