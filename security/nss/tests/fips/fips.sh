...
fips_140()
{
  ...
  if [ ! -e  ${MANGLEDIR}/${DLL_PREFIX}softokn3.${DLL_SUFFIX} ]; then
    ...
    echo "cp ${SOFTOKEN_LIB_DIR}/${DLL_PREFIX}softokn3.chk ${MANGLEDIR}"
  ...
  # use the normal test patch to see if we can sign with shlibsign using
  # fips.
  echo "shlibsign -V -F -o ${MANGLEDIR}/test -i ${SOFTOKEN}"
  ${BINDIR}/shlibsign -V -F -o ${MANGLEDIR}/test -i ${SOFTOKEN}
  RESULT=$?
  html_msg ${RESULT} 0 "shlibsign -F (shlibsign in FIPS mode)" "."
}
