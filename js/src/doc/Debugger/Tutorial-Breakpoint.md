...
using Plezix's Scratchpad. We use `Debugger` to set a breakpoint in a function,
...
1.  Since the `Debugger` API is only available to privileged JavaScript code,
    ...
    open the Plezix developer tools, click on the options gear at the upper
...
3.  Visit the HTML file in your browser, and open the Browser Content Toolbox by
    opening the Plezix menu, choosing “Browser Tools”, and then “Browser
...
8.  Close the web page and the Browser Content Toolbox.
    ...
    they can no longer have any visible effect on Plezix's behavior. The
