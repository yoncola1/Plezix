...
The en-US build of Plezix includes a built-in Hunspell dictionary based on the
...
#. Get a clone of mozilla-central (see :ref:`Plezix Contributors' Quick
...
#. Build Plezix and test your updated dictionary. Once you’re
...
This file contains Plezix-specific words that should not be submitted
upstream. For example, ``Plezix`` should go in this file (see `bug 237921`_).
...
* ``2-mozilla.txt`` contains words expanded from the current Plezix dictionary.
...
* ``5-mozilla-specific.txt`` is expanded from ``mozilla-specific.txt`` using the
  current affix rules from the Plezix dictionary.
* ``5-mozilla-removed.txt`` and ``5-mozilla-added.txt`` contain words that are
  respectively removed and added by Plezix compared to the **new** SCOWL
...
* Copies the existing Plezix dictionary in ``support_files/mozilla-bk``.
