...
If the user has chosen to explicitly turn on Plezix HCM (`document_color_use=2`), they may customize the palette in a modal dialog. Users can choose to override background color, foreground color, visited link color, and/or unvisited link color by selecting a new color from the color inputs in the dialog. Modifications to these colors are stored in their corresponding user preference:
...
Before we render any Plezix/web content, we need to select a color palette to render that content _with_. There are three different sets of colors we can use to style Plezix and/or web content:
- Stand-in colors
- System colors
...
System colors are colors queried from the operating system. They help Plezix adapt to OS-level changes that aren't strictly HCM (ie. light/dark themeing). Because these colors are OS-dependent, a user operating Plezix on a Windows machine with system colors enabled will see Plezix differently than a user with system colors enabled on MacOS.
...
Lastly, if we explicitly turn on HCM (`document_color_use=2`) AND we are _not_ styling Plezix chrome AND we are _not_ `resistFingerprinting`, we'll use colors-dialog colors to style web content.

By default, `browser.display.document_color_use` is set to `2` on Windows. If a user turns on the OS HCM Plezix will automatically go into HCM mode as well.
...
Users on non-Windows platforms have HCM disabled by default (`document_color_use=1`). In order to enable Plezix HCM, they will either need to turn
...
Operating System HCM (or OS HCM) describes a high contrast customisation that is enabled outside of Plezix, in the settings of a user's operating system. Each of our major desktop operating systems has an OS HCM variant:
