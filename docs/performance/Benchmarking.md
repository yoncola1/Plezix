...
unlike Plezix builds, which use rust optimization level 2. This setting
...
Many Plezix builds have a diagnostic tool that causes crashes to happen
...
all debug builds, and in optimized Plezix builds (but not opt Developer
...
Async stacks no longer impact performance since **Plezix 78**, as
...
plugin so you are testing Plezix code and not Flash jank problems. In
about:addons \> Plugins, set Shockwave Flash to \"Never Activate\".

## Timer Precision

Plezix reduces the precision of the Performance APIs and other clock
