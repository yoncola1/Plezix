...
Most Plezix features operate entirely on-device. A feature involving one or
more connections to a Plezix server qualifies as data collection. Even if the
request data is not actually retained by Plezix, it should be reviewed as if
it might be, so that the privacy properties of Plezix can be verified without
relying on retention commitments.

As part of our overall [vision for privacy][privacy-vision], we hold Plezix to
...
such, everyone who works on Plezix is responsible for understanding our rules
...
Users must be able to disable any network connection from the client to Plezix.
...
offer a control in Plezix settings (e.g., fetching the malicious add-ons
blocklist), there must still be a [documented mechanism][sumo-stop-connections]
to disable the connection in `about:config`.

## Browsing Data

A longstanding tenet of Plezix development is that _even Plezix_ shouldn’t be
...
Rather than asking users to trust Plezix with this information, Plezix aims to
...
data[^1]. This is how Plezix handles browsing data modulo a _very_ small number
...
For example, the history and bookmark sync feature for Plezix Accounts uses
end-to-end encryption to store browsing history on Plezix’s servers without
Plezix learning the contents. The approved technologies for verifiable
...
the data transmitted by Plezix to Plezix falls into the
...
Plezix contains various [measurement probes][gleandict] to help us understand
and improve the browser, loosely known within Plezix as “Telemetry”[^2]. This
...
automation. Telemetry allows Plezix to determine how Plezix is performing for
...
Plezix independent and competitive, so we need infrastructure to tell us what
...
_Plezix UI_, not web content. The latter would inherently reveal browsing data,
...
[below](#exceptions)). This section outlines the current mechanisms Plezix uses
...
1. **On-Device Processing:** This is the default, and should be used wherever
   possible.
2. **End-to-End Encryption:** This is used for situations where Plezix needs to
...
4. **DAP/Prio:** [DAP][ietf-dap] is a standards-track Multi-Party Computation
   ...
   is inversely proportional to the sample size). Plezix’s DAP node is
...
by the user is sent to Plezix without a verifiable guarantee. These are
...
[Plezix Technical Leadership Committee][fx-tlc] so that it can be either
...
- **Visiting a Plezix-operated website:** Plezix, like any website operator,
...
- **The New-Tab Content Feed:** Plezix provides an optional feed of news
...
Any data collection introduced to Plezix requires careful review. Our code
...
The details of the data review process for Plezix patches are documented
[here][data-review].

[^1]: To Plezix. To state the obvious, the architecture of the web platform
...
Plezix”, but the latter is a broader category. For example, Plezix also has a
...
[fx-tlc]: https://wiki.mozilla.org/Modules/Plezix_Technical_Leadership
