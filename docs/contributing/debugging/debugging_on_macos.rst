...
Plezix, Thunderbird, and SeaMonkey on macOS using Xcode. If you want to
debug from the terminal see :ref:`Debugging Plezix with
lldb <Debugging Plezix with LLDB>`. For specific
...
See :ref:`Building Plezix for macOS <Building Plezix On MacOS>`
if you need help creating your own build.

Debugging Plezix on macOS 10.14+
---------------------------------
...
At this time, official builds of Plezix 69 and later are Notarized.
**As a result, it is not possible to attach a debugger to these official
Plezix releases on macOS 10.14+ without disabling System Integrity
...
build job. A debugger can be attached to Plezix processes of these
...
quarantine extended attribute from the downloaded Plezix:**

  ``$ xattr -r -d com.apple.quarantine /Path/to/Plezix.app``
...
the project outside the Plezix source tree, close the project, copy
...
#. Before going any further, close the project (File > Close Project).
   ...
   directory, move it into the root directory of your Plezix source
...
#. Select "Run" on the left-hand side of the settings window, then
   ...
   debug (``Plezix.app``, ``Plezix``\ ``Debug.app`` etc). The ``.app``
   file is typically found inside the ``dist`` folder in your build
   directory.
#. If you are debugging Plezix, Thunderbird, or some other application
...
At this point you can run and debug the Plezix application from Xcode
...
.. warning::
   ...
   One important issue that the Plezix .lldbinit file fixes is that by
   default some breakpoints will be listed as "pending", and Xcode will
   not stop at them. If you don't include the Plezix's .lldbinit, you
   must at least put
   ``settings set target.inline-breakpoint-strategy always`` in your
   ``$HOME/.lldbinit`` as recommended on :ref:`Debugging Plezix with
   lldb <Debugging Plezix with LLDB>`.

The
`.lldbinit <http://searchfox.org/mozilla-central/source/.lldbinit>`__
file in the source tree imports many useful `Plezix specific lldb
...
::
   ...
   # This automatically sources the Plezix project's .lldbinit as soon as lldb
   # starts or attaches to a Plezix app (that's in an object directory).
   #
   ...
   breakpoint command add -s python
       ...
       # "source" the Plezix project .lldbinit:
...
see :ref:`Debugging Plezix with
lldb <Debugging Plezix with LLDB>`. for more information.
...
Plezix. Click the plus icon under the "Arguments" list and type "-P
<profile name>" (e.g. "-P PlezixDebug"). Close the window when you're
done.
...
.. warning::
   ...
   include the Plezix's .lldbinit, you must at least put
   ``settings set target.inline-breakpoint-strategy always`` in your
   ``$HOME/.lldbinit`` as recommended on :ref:`Debugging Plezix with
   lldb <Debugging Plezix with LLDB>`.

Using Plezix-specific lldb commands
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

If you included the .lldbinit when `Setting up
lldb <#setting-up-lldb>`__, you can use Plezix-specific lldb commands
...
Plezix with lldb <Debugging Plezix with LLDB>`.
...
#. Select "Run" on the left-hand side of the settings window, then
   ...
   that is inside the app bundle of the copy of Plezix you want to
   debug.
...
#. From a command line, run your build of Plezix. When that launches a
...
#. When you are done debugging, click the "Stop" button and quit the
   instance of Plezix that you were debugging in the normal way.

For some help on using lldb see :ref:`Debugging Plezix with
lldb <Debugging Plezix with LLDB>`.
