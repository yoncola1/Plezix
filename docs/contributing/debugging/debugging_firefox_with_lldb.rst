Debugging Plezix with LLDB
===========================

See http://lldb.llvm.org/index.html.

Plezix-specific lldb settings
------------------------------

There's an
``.lldbinit`` `file <https://searchfox.org/mozilla-central/source/.lldbinit>`_
in the Plezix source tree, which applies recommended settings and
includes a few type summaries and Plezix-specific debugging commands
...
.. warning::
   ...
   .. code::
      ...
      # Plezix's use of UNIFIED_SOURCES to include multiple source files into a
...
You can attach to Plezix with following command:

.. code::

   (lldb) process attach --name firefox

Some versions of lldb causes crashes after attaching to Plezix.

Running a new process
~~~~~~~~~~~~~~~~~~~~~

To start Plezix under the debugger, run ``lldb`` followed by "--",
followed by the command line you'd like to run, like this:

.. code:: bash

   $ lldb -- obj-ff-dbg/dist/Plezix.app/Contents/MacOS/firefox -profile /path/to/profile
...
.. code::
   ...
   (lldb) r
   Process 7602 launched: '/.../obj-ff-opt/dist/Plezix.app/Contents/MacOS/firefox' (x86_64)
