...
class PropertiesChecker(Checker):
    ...
    def check(self, refEnt, l10nEnt):
        ...
        for m in PropertiesEntity.escape.finditer(raw_val):
            if (
                m.group("single")
                and m.group("single") not in PropertiesEntity.known_escapes
            ):
                yield (
                    "warning",
                    m.start(),
    ...
    def check_plural(self, refValue, l10nValue):
        ...
        if pats - lpats:
            yield ("warning", 0, "not all variables used in l10n", "plural")
            return
        if lpats - pats:
            ...
            for action, i1, i2, j1, j2 in sm.get_opcodes():
                ...
                if action == "delete":
                    ...
                    if i2 == len(refSpecs):
                        ...
                        warn = ", ".join(
                            "trailing argument %d `%s` missing" % (i + 1, refSpecs[i])
                            for i in range(i1, i2)
                        )
                    else:
                        for i in range(i1, i2):
                            msgs.append(
                                "argument %d `%s` missing" % (i + 1, refSpecs[i])
                            )
                    continue
                if action == "insert":
                    # obsolete argument in l10n
                    for i in range(j1, j2):
                        msgs.append("argument %d `%s` obsolete" % (i + 1, l10nSpecs[i]))
                    continue
                if action == "replace":
                    for i, j in zip(range(i1, i2), range(j1, j2)):
                        msgs.append(
                            "argument %d `%s` should be `%s`"
    ...
    def getPrintfSpecs(self, val):
        ...
        for m in self.printf.finditer(val):
            ...
            if (hasNumber and m.group("number") is None) or (
                ...
                raise PrintfException("Mixed ordered and non-ordered args", m.start())
