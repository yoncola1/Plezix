...
"""
    ...
    def known_entities(self, refValue):
        if self.__known_entities is None and self.reference is not None:
            ...
            for ent in self.reference.values():
                self.__known_entities.update(self.entities_for_value(ent.raw_val))
        return (
            self.__known_entities
            if self.__known_entities is not None
    ...
    def check(self, refEnt, l10nEnt):
        ...
        try:
            parser.parse(
                BytesIO(
                    self.tmpl % (entities.encode("utf-8"), refValue.encode("utf-8"))
                )
            )
            # also catch stray %
            parser.parse(
            ...
            e  # noqa
            yield ("warning", (0, 0), "can't parse en-US value", "xmlparse")
        ...
        try:
            parser.parse(
                BytesIO(
                    self.tmpl % (_entities.encode("utf-8"), l10nValue.encode("utf-8"))
                )
            )
            # also catch stray %
    ...
    def processAndroidContent(self, val):
        ...
        for m in stray_quot.finditer(val):
            if len(m.group(0)) % 2:
                ...
                if m.group(1) == '"':
                    msg = (
                        'Quotes in Android DTDs need escaping with \\" '
                        "or \\u0022, or put string in apostrophes."
                    )
                else:
                    msg = (
                        "Apostrophes in Android DTDs need escaping with "
