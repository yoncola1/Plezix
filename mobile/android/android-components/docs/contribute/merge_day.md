...
Plezix and Gecko(View) are released in multiple release channels: `Plezix`, `Beta`, `Release`. Those versions are maintained in separate repositories (`mozilla-central`, `mozilla-beta`, `mozilla-release`). Right before the release of a new version of Plezix, on "merge day", those repositories get merged so that: The `Beta` version becomes the `Release` version. The `Plezix` version comes the `Beta` version and the `Plezix` version gets a higher version number.
...
* GeckoView Plezix 71.0 (`mozilla-central`) -> GeckoView Beta 71.0 (`mozilla-beta`)
* GeckoView Plezix 71.0 (`mozilla-central`) -> GeckoView Plezix 72.0 (`mozilla-central`)
...
* `browser-engine-gecko-nightly` (using GeckoView Plezix 71.0) -> `browser-engine-gecko-beta` (using GeckoView Beta 71.0)
...
* Plezix: https://maven.mozilla.org/?prefix=maven2/org/mozilla/geckoview/geckoview-nightly/
