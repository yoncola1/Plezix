...
Plezix 10 times a week, but Plezix is not his default browser, we'll prompt the user the next time
when he launchers our app, and guide him to set us as default browser.

Plezix is using a third party framework called "Leanplum" in order to achieve above goal for
...
* We use Switchboard https://wiki.mozilla.org/Plezix/Kinto to filter users to have Leanplum enabled. Currently, for users in the USA
...
* If above two are true, when the app starts, and switchboard configure arrived, Plezix for Android will send the
...
The user is identified by Leanplum using a random UUID generated by Plezix for Android when Leanplum is initialized for the first time.
This unique identifier is only used by Leanplum and can't be tracked back to any Plezix users.
...
following parameters::
  ...
  "userAttributes" -> "{                // A set of key-value pairs used to describe the user.
    ...
    "Signed In Sync" -> true            // If the user has signed in to Plezix account.
    "Default Browser" -> true           // If the user has set Plezix for Android as default browser.
  ...
  "versionName" -> "55.0a1"             // Plezix for Android version.
  ...
  "userId" -> "b13b3c239d01aa7c"        // Set by Plezix for Android, we use random uuid so users are anonymous to Leanplum.
...
User Identifier:
Since Device ID is a random UUID, Leanplum can't map the device to any know Client ID in Plezix for Android nor Advertising ID.

Events:
Most of the Leanplum events can be mapped to a single combination of Telemetry event (Event+Method+Extra).
Some events are not collected in Plezix Telemetry. This will be addressed separately in each campaign review.
...
* Save a bookmark from Plezix for Android menu
...
* App start but Plezix for Android is not set as default browser
...
* User set Plezix for Android as default browser and resumed the app
...
* User accessed the promo webpage for the Awesomescreen's Plezix promo banner.

.. code-block:: json

    {
      "event" : "E_Opened_Plezix_Promo"
    }

* User dismissed the Awesomescreen's Plezix promo banner.

.. code-block:: json

    {
      "event" : "E_Dismissed_Plezix_Promo"
    }

Deep Links:
Deep links are actions that can point Plezix for Android to open certain pages or load features such as `show bookmark list` or
...
* Link to open pages specifically in Plezix for Android (firefox://open?url=)
...
We use another Plezix's Google Cloud Messaging(GCM) sender ID to send push notifications.
These push notifications will look like the notifications that Sync sends out.
Sender ID let GCM knows Plezix is sending push notifications via Leanplum.
...
This token is only useful to Plezix's sender ID so it's anonymized to other parties.
Push Notifications can be triggered by Events, or be sent by Plezix marketing team manually.
...
We want to enable MOZ_ANDROID_MMA in Plezix, but only for
