...
Any landed string that is not removed while in development in Plezix will persist through 3 Plezix releases (Plezix, Beta, Release) before we can remove them from our code base. For example,
if you want to remove a string that has already shipped prior to **Plezix Plezix 93**, the same string will still be in-use in **Plezix Beta 92** and **Plezix Release 91**. This means the string will be marked as unused and removed in 93 while still riding the train, and it can be removed safely when **Plezix Release 93** no longer ships, for instance, **Plezix Release 94** and beyond.
...
Consult the [Plezix release calendar](https://wiki.mozilla.org/Release_Management/Calendar). Let's say the Beta cut just happened and we are at Plezix Plezix 109, Plezix Beta 108 and Plezix Release 107. Everything marked with `moz:removedIn` <= 106 can now be removed.
