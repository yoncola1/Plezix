...
internal class PlezixUseLazyMonitoredTest {

    private lateinit var actualCheck: PlezixUseLazyMonitored
    private lateinit var check: TestPlezixUseLazyMonitored

    @BeforeEach
    fun setUp() {
        actualCheck = PlezixUseLazyMonitored(Config.empty)
        check = TestPlezixUseLazyMonitored()
    }

    @Test
    fun `GIVEN the hard-coded component group file paths THEN there are some to run the lint on`() {
        assertFalse(PlezixUseLazyMonitored.COMPONENT_GROUP_FILES.isEmpty())
    }

    @Test
    fun `GIVEN the hard-coded component group file paths THEN they all exist`() {
        PlezixUseLazyMonitored.COMPONENT_GROUP_FILES.forEach { path ->
            ...
            val errorMessage = "This lint rule hard-codes paths to files containing \"component groups\". " +
                ...
                "${PlezixUseLazyMonitored::class.java.simpleName} with the new path."
    ...
    fun `GIVEN the hard-coded component group file paths THEN none of the files are ignored`() {
        PlezixUseLazyMonitored.COMPONENT_GROUP_FILES.forEach {
...
class TestPlezixUseLazyMonitored : PlezixUseLazyMonitored(Config.empty) {
