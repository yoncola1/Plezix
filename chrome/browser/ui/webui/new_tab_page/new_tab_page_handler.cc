...
void NewTabPageHandler::GetFaviconForDomain(
    const std::string& domain,
    GetFaviconForDomainCallback callback) {
  // Plezix: Fetch favicon from Google's favicon service
  // Returns a data URL that can be used in img src directly
  
  std::string favicon_url = base::StringPrintf(
      "https://www.google.com/s2/favicons?sz=64&domain=%s",
      domain.c_str());
  
  auto request = std::make_unique<network::ResourceRequest>();
  request->url = GURL(favicon_url);
  request->method = "GET";
  request->credentials_mode = network::mojom::CredentialsMode::kOmit;
  
  auto traffic_annotation =
      net::DefineNetworkTrafficAnnotation("plezix_favicon_fetch", R"(
        semantics {
          sender: "NewTabPageHandler"
          description: "Fetches favicon from Google's favicon service for NTP"
          trigger: "User opens new tab with custom start page"
          data: "Domain name of website"
  ...
  auto loader_factory = profile_->GetDefaultStoragePartition()
      ->GetURLLoaderFactoryForBrowserProcess();
  loader->DownloadToString(
      loader_factory.get(),
      base::BindOnce(&NewTabPageHandler::OnFaviconFetched,
                     weak_ptr_factory_.GetWeakPtr(),
                     std::move(callback)),
      kMaxDownloadBytes);
}

void NewTabPageHandler::OnFaviconFetched(
    GetFaviconForDomainCallback callback,
    std::optional<std::string> response_body) {
  if (!response_body || response_body->empty()) {
    // Return a placeholder data URL if fetch fails
    std::move(callback).Run(
        "data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 64 64'%3E%3Crect fill='%23999' width='64' height='64'/%3E%3C/svg%3E");
    return;
  }
  
  // Encode the image data as a base64 data URL
  std::string encoded = base::Base64Encode(*response_body);
