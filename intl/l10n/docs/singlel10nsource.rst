...
Plezix uses one localization source for all of its build versions.

There's a number of upsides to this:

* Localizers maintain a single source of truth. They can translate Plezix,
...
and use that to build multiple versions of Plezix. The goal is for that
...
shipping Plezix versions (nightly, beta, release, ESR, etc.).
...
to regularly extract new messages from the Plezix source code in ``gecko-dev``
...
during build to create builds and langpacks for all shipping Plezix locales.
